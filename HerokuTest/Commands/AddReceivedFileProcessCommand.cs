...
{
    public class AddReceivedFileProcessCommand: BaseCommand
    {
        private readonly ITrackingService _trackingService;
        private readonly TelegramBotClient _botClient;

        public AddReceivedFileProcessCommand(TelegramBot telegramBot, ITrackingService trackingService)
        {
            _trackingService = trackingService;
            _botClient = telegramBot.GetBot().Result;
        }

        public override string Name => CommandNames.AddReceivedFileProcessCommand;
        ...
        public override async Task ExecuteAsync(Update update)
        {
            if (update.Type == UpdateType.Message)
            {
                ...
                if (msg.Type == MessageType.Document)
                {
                    ...
                    await _trackingService.SetReceivedTrackingRange(trackcodes, receivedDate);
